...
def run_fixed_window_backtest(
    ...
    for i, row in df.iterrows():
        ...
        for pos in positions:
            ...
            if use_ladder_exits and can_sell_now:
            ...
            triggered_rule = None
            if can_sell_now:
                triggered_rule = next((r for r in exit_rules if _eval_rule(r, row, prev, pos_ctx)), None)
            if triggered_rule is not None and pos["shares"] > 0:
                if use_ladder_exits:
                    unsold_steps = [step for step in ladder_steps if step not in pos["sold_steps"]]
                    for step in unsold_steps:
                        if pos["shares"] <= 0:
                            break
                        shares_to_sell = min(pos["original_shares"] * ladder_fraction, pos["shares"])
                        proceeds = shares_to_sell * current_close
...
def run_fixed_window_optimization(
    ...
    for pos_above, profit_target, time_stop in product(
