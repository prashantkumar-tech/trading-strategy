"""MES trend slot ladder study page."""
...
from backtest.mes_trend_slot_ladder import DEFAULT_PARAMS, run_backtest
from dashboard.shared import render_sidebar
from data.database import get_date_range, load_prices

PAGE_DEFAULTS = {
    **DEFAULT_PARAMS,
    "max_open_campaigns": int(DEFAULT_PARAMS.get("max_open_campaigns", 4)),
    "daily_stop_limit": int(DEFAULT_PARAMS.get("daily_stop_limit", 2)),
}
...
def _build_yearly_table(initial_capital: float, params: dict, latest_date: pd.Timestamp) -> pd.DataFrame:
    ...
    mes_full_df = load_prices("MES", start="2020-01-01", end=str(latest_date.date()), bar_size="5m", source="databento")
    mes_full_result = run_backtest(mes_full_df, params=params, initial_capital=initial_capital)
    ...
    for year in range(2020, latest_date.year + 1):
        ...
        rows.append({
            ...
            "Trade Exits": int(m["num_trades"]),
            "Win Rate": f"{float(m['win_rate_pct']):.2f}%",
            "Campaigns": int(s["campaigns_opened"]),
            "Skipped Lower Slots": int(s["skipped_lower_slots"]),
            "Skipped Daily Stop": int(s.get("skipped_daily_stop_slots", 0)),
    ...
    total_mes_return = float(mes_full_result["metrics"]["total_return_pct"]) if mes_full_result else None
    mes_last_result = float(mes_full_result["final_value"]) if mes_full_result else None

    total_spy_return = None
    if not spy_df.empty:
        total_spy_return = (float(spy_df["close"].iloc[-1]) / float(spy_df["close"].iloc[0]) - 1) * 100

    total_qqq_return = None
    if not qqq_df.empty:
        total_qqq_return = (float(qqq_df["close"].iloc[-1]) / float(qqq_df["close"].iloc[0]) - 1) * 100

    rows.append({
        "Year": "Total",
        "Period End": str(latest_date.date()),
        "Final Value": f"${mes_last_result:,.2f}" if mes_last_result is not None else "N/A",
        "MES Return": f"{float(total_mes_return):+.2f}%" if total_mes_return is not None else "N/A",
        "Sharpe": "N/A",
        "Max DD": "N/A",
        "Trade Exits": "N/A",
        "Win Rate": "N/A",
        "Campaigns": "N/A",
        "Skipped Lower Slots": "N/A",
        "Skipped Daily Stop": "N/A",
        "Target 1 Hit Rate": "N/A",
        "Runner Win Rate": "N/A",
        "SPY Return": f"{float(total_spy_return):+.2f}%" if total_spy_return is not None else "N/A",
        "QQQ Return": f"{float(total_qqq_return):+.2f}%" if total_qqq_return is not None else "N/A",
        "MES - SPY": f"{float(total_mes_return - total_spy_return):+.2f}%" if total_mes_return is not None and total_spy_return is not None else "N/A",
        "MES - QQQ": f"{float(total_mes_return - total_qqq_return):+.2f}%" if total_mes_return is not None and total_qqq_return is not None else "N/A",
    })
    return pd.DataFrame(rows)


st.set_page_config(page_title="MES Trend Slot Ladder", layout="wide", page_icon="📈")
st.title("📈 MES Trend Slot Ladder")
st.caption("Version 2: 30-minute MES long campaigns with a 10-point stop, 20-point first target, overlap cap, lower-slot skipping, and a daily kill switch after repeated stopped campaigns.")
...
if "mes_trend_slot_ladder_range" not in st.session_state:
    st.session_state["mes_trend_slot_ladder_range"] = (default_start_date, available_end)

range_value = st.date_input(
    "Date range",
    value=st.session_state["mes_trend_slot_ladder_range"],
    min_value=available_start,
    max_value=available_end,
    key="mes_trend_slot_ladder_range",
)
...
with pc1:
    contracts_per_campaign = st.number_input("Contracts per campaign", min_value=1, max_value=4, value=int(PAGE_DEFAULTS["contracts_per_campaign"]), step=1)
with pc2:
    target1_points = st.number_input("Target 1 (pts)", min_value=1.0, max_value=100.0, value=float(PAGE_DEFAULTS["target1_points"]), step=0.5)
with pc3:
    stop_loss_points = st.number_input("Stop loss (pts)", min_value=1.0, max_value=100.0, value=float(PAGE_DEFAULTS["stop_loss_points"]), step=0.5)
with pc4:
    runner_trail_points = st.number_input("Runner trail (pts)", min_value=1.0, max_value=100.0, value=float(PAGE_DEFAULTS["runner_trail_points"]), step=0.5)

pc5, pc6, pc7, pc8 = st.columns(4)
with pc5:
    entry_interval_minutes = st.selectbox("Entry interval", options=[15, 30, 60], index=[15, 30, 60].index(int(PAGE_DEFAULTS["entry_interval_minutes"])))
with pc6:
...
pc9, pc10, pc11, _pc12 = st.columns(4)
with pc9:
    skip_lower_entries = st.checkbox("Skip lower-priced re-entries", value=bool(PAGE_DEFAULTS["skip_lower_entries"]))
with pc10:
    max_open_campaigns = st.number_input("Max open campaigns", min_value=1, max_value=8, value=int(PAGE_DEFAULTS["max_open_campaigns"]), step=1)
with pc11:
    daily_stop_limit = st.number_input("Daily stop limit", min_value=1, max_value=10, value=int(PAGE_DEFAULTS["daily_stop_limit"]), step=1)
...
params = {
    **PAGE_DEFAULTS,
    ...
    "runner_trail_points": float(runner_trail_points),
    ...
    "skip_lower_entries": bool(skip_lower_entries),
    "max_open_campaigns": int(max_open_campaigns),
    "daily_stop_limit": int(daily_stop_limit),
}

st.subheader("Rule Summary")
st.caption(
    "Each valid slot opens a new 2-lot campaign on the next bar open. "
    "Lot 1 exits at the fixed target, lot 2 moves to breakeven once target 1 hits and then trails upward only. "
    "If enabled, any slot priced below the last accepted entry for that day is skipped. "
    "New entries also stop for the day after the configured number of stopped campaigns, and overlap is capped by the max-open-campaigns setting."
)
...
result_key = f"mes_trend_slot_ladder_{start_date}_{end_date}_{params_key}"
if st.button("▶ Run MES Trend Slot Ladder", type="primary"):
    with st.spinner("Running MES trend slot ladder backtest…"):
        result = run_backtest(df, params=params, initial_capital=initial_capital)
    st.session_state[result_key] = result

result = st.session_state.get(result_key)
if result is None:
    st.info("Press **▶ Run MES Trend Slot Ladder** to see results.")
    st.stop()
...
skipped_daily_stop_slots = int(summary.get("skipped_daily_stop_slots", 0))
...
s4.metric("Skipped Lower Slots", f"{summary['skipped_lower_slots']}")
s5.metric("Skipped Daily Stop", f"{skipped_daily_stop_slots}")
s6.metric("Max Concurrent", f"{summary['max_concurrent_campaigns']}")
...
t3.metric("Avg Hold (bars)", f"{summary['avg_hold_bars']:.1f}")

st.subheader("MES Price and Events")
price_fig = make_subplots(rows=2, cols=1, shared_xaxes=True, row_heights=[0.75, 0.25], subplot_titles=("Price / Session Open", "Volume"))
...
if campaigns:
    ...
    cdf["entry_price"] = cdf["entry_price"].map(lambda x: f"{x:,.2f}")
    cdf["target1_price"] = cdf["target1_price"].map(lambda x: f"{x:,.2f}")
    cdf["initial_stop_price"] = cdf["initial_stop_price"].map(lambda x: f"{x:,.2f}")
    cdf["realized_pnl"] = cdf["realized_pnl"].map(lambda x: f"${x:+,.2f}")
    cdf["runner_realized"] = cdf["runner_realized"].map(lambda x: f"${x:+,.2f}")
    st.dataframe(cdf, use_container_width=True, hide_index=True)
else:
    st.info("No campaigns were opened in the selected range.")
...
st.caption("Uses the current MES Trend Slot Ladder settings across each calendar year from 2020 through the latest available MES date. 2026 is partial year through the latest available data.")
